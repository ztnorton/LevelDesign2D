...
int main(int argc, char* argv[]) {
	...
	// ******************* Create the melee texture - START ******************

	// Load Room BITMAP to a SDL Texture
	SDL_Texture *melee = IMG_LoadTexture(renderTarget,
			(images_dir + "enemyMelee.png").c_str());
	...
	int w1, h1;
	SDL_QueryTexture(melee, NULL, NULL, &w1, &h1);
	melee0Pos.w = w1;
	melee0Pos.h = h1;

	// ******************* Create the melee texture - END *********************

	// ******************* Create the Pickups texture - START ******************

	// Load Room BITMAP to a SDL Texture
	SDL_Texture *ammoP = IMG_LoadTexture(renderTarget,
			(images_dir + "ammoP.png").c_str());
	SDL_Texture *healthP = IMG_LoadTexture(renderTarget,
			(images_dir + "heartP.png").c_str());
	SDL_Texture *keyP = IMG_LoadTexture(renderTarget,
			(images_dir + "keyP.png").c_str());
	SDL_Texture *fireP = IMG_LoadTexture(renderTarget,
			(images_dir + "fireP.png").c_str());
	SDL_Texture *daggerP = IMG_LoadTexture(renderTarget,
			(images_dir + "daggerP.png").c_str());
	...
	int spacing = 200;
	// Set the x, y, width and height SDL Rectangle values
	ammoPpos.x = spacing; spacing += 75;
	ammoPpos.y = 270;
	healthPpos.x = spacing; spacing += 75;
	healthPpos.y = 270;
	keyPpos.x = spacing; spacing += 75;
	keyPpos.y = 270;
	firePpos.x = spacing; spacing += 75;
	firePpos.y = 270;
	daggerPpos.x = spacing; spacing += 75;
	daggerPpos.y = 270;
	...
	// ******************* Create the Pickups texture - END *********************
	...
	// ******************* Create the room (Walls/Floors) texture - START *******************

	// load the surface into the texture
	SDL_Texture *wallsF0 = IMG_LoadTexture(renderTarget,
			(images_dir + "palaceWF0.png").c_str());
	// load the surface into the texture
	SDL_Texture *wallsF1 = IMG_LoadTexture(renderTarget,
			(images_dir + "palaceWF1.png").c_str());
	// load the surface into the texture
	SDL_Texture *wallsF2 = IMG_LoadTexture(renderTarget,
			(images_dir + "palaceWF2.png").c_str());
	// load the surface into the texture
	SDL_Texture *wallsF3 = IMG_LoadTexture(renderTarget,
			(images_dir + "palaceWF3.png").c_str());
	// load the surface into the texture
	SDL_Texture *wallsF4 = IMG_LoadTexture(renderTarget,
			(images_dir + "dungeonWF0.png").c_str());
	...
	SDL_QueryTexture(wallsF0, NULL, NULL, &w, &h);
	wallsFPos.w = w;
	wallsFPos.h = h;
	SDL_QueryTexture(wallsF1, NULL, NULL, &w, &h);
	wallsFPos1.w = w;
	wallsFPos1.h = h;
	SDL_QueryTexture(wallsF2, NULL, NULL, &w, &h);
	wallsFPos2.w = w;
	wallsFPos2.h = h;
	SDL_QueryTexture(wallsF3, NULL, NULL, &w, &h);
	wallsFPos3.w = w;
	wallsFPos3.h = h;

	SDL_QueryTexture(wallsF4, NULL, NULL, &w1, &h1);
	wallsFPos4.w = w1;
	wallsFPos4.h = h1;
...
//		SDL_RenderCopy(renderTarget, bkgd1, NULL, &roomPos4);
		...
		// Copy the roomTexture to the RenderTarget using the roomPos to place it in position
//		SDL_RenderCopy(renderTarget, wallsF0, NULL, &wallsFPos);
//		SDL_RenderCopy(renderTarget, wallsF1, NULL, &wallsFPos1);
//		SDL_RenderCopy(renderTarget, wallsF2, NULL, &wallsFPos2);
//		SDL_RenderCopy(renderTarget, wallsF3, NULL, &wallsFPos3);
//		SDL_RenderCopy(renderTarget, wallsF4, NULL, &wallsFPos4);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, ammoP, NULL, &ammoPpos);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, healthP, NULL, &healthPpos);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, keyP, NULL, &keyPpos);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, daggerP, NULL, &daggerPpos);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, fireP, NULL, &firePpos);

		// Copy the playerTexture to the RenderTarget using the roomPos to place it in position
		SDL_RenderCopy(renderTarget, player, NULL, &playerPos);

		// Copy the rangedTexture to the RenderTarget using the roomPos to place it in position
//		SDL_RenderCopy(renderTarget, melee, NULL, &melee0Pos);
